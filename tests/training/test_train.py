...
class TestTrain:
    ...
    def test_train_updates_model_parameters(
        ...
        pre_train_params = [param.data.clone().to(device) for param in model.parameters()]

        _call_train()

        post_train_params = [param.data.clone().to(device) for param in model.parameters()]
    ...
    def test_train_calls_model_forward_with_correct_inputs(
        ...
        assert x.shape[1] == CONTEXT_SIZE - 1  # sequence length is shifted for next-token prediction
    ...
    def test_train_time_taken_is_accurate(self, _call_train):
        ...
        assert abs(time_taken - training_state.time_taken) < 0.01  # Allow 10ms tolerance
